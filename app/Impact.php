...
class Impact
{
    ...
    public function getSuggestionType() {
        ...
        if(preg_match("/(modifications horaires|horaires modifiés|Modifications d'horaires|Changement d'horaires?|modification horaire|Changement de service|Modification du plan de transport)/i", $this->getTitle())) {
        ...
        if(preg_match("/Les horaires de départ ou de passage en gare de certains trains( de la ligne R | )peuvent être avancés/", $this->getMessagePlainText())) {
            return self::TYPE_AUCUNE;
        }
