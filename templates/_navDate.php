...
<select id="select-day" onchange="document.location.href=this.value; this.value='';" autocomplete="off">
    <option style="display: none;" value="" selected="selected"><?php if($period->isToday() && $period instanceof Day):?>Aujourd'hui<?php else: ?><?php echo $period->getDateStartLabel(); ?><?php endif; ?></option>
    <?php foreach(View::getDatesChoices() as $group => $choices): ?>
    <optgroup label="<?php echo $group ?>">
    <?php foreach($choices as $dateChoiceKey => $dateChoiceLibelle): ?>
    <option value="<?php echo View::url("/".$dateChoiceKey."/".$mode.".html") ?>"><?php if($period->getDateStartKey() == $dateChoiceKey):?>🔘<?php else: ?>⚪<?php endif; ?> <?php echo $dateChoiceLibelle ?></option>
    <?php endforeach; ?>
    </optgroup>
    <?php endforeach; ?>
</select>
<?php if($period->isToday()):?>
<a class="disabled">➡️</a>
<?php else: ?>
